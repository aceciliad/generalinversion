...
c    &           unit23=(a23,create,text)//")
      ...
      open(2,file='crfl.psv',access='sequential',form='formatted')
      rewind 5
      rewind 6
      rewind 2
      z(0)=0.
...
c     bedeutung der schalter
 ...
 679  format(' seismogram for acceleration')
      if(iss(5).eq.0) then
      open(3,file='crfl.sh',access='sequential',form='formatted')
      rewind 3
      endif
      if(iss(6).eq.1) then
...
c
      call output(dentif,nent,iouts)
      call output(dentif,nent,ioutsh)
      if(iss(6).eq.1) then
...
c NS_CHANGE
  ...
  100 format(10x,a42)
      ...
      write(iouts,110) nlay,mdeck,iso,iss4
  110 format(4i5,5x,'nlay, mdeck,  iso,  iss(4)')
      nl=nlay-1
      md=mdeck+1
      if(mdeck.gt.0) then
      write(iouts,120) (z(i),d(i),ar(i),br(i),rho(i), i=1,mdeck)
  120 format(5f10.4)
      end if
      if(md.gt.nl) go to 1
      write(iouts,130) (z(i),d(i),ar(i),br(i),rho(i), i=md,nl)
  130 format(5f10.4,5x,'refl. zone')
    1 write(iouts,140) ar(nlay),br(nlay),rho(nlay)
  140 format(20x,3f10.4,5x,'refl. zone')
      write(iouts,150) nent
  150 format(i5,' zahl der seismogramme = zahl der distanzen')
      write(iouts,160) (x(i), i=1,nent)
  160 format(7f10.3)
      write(iouts,170) vred,tmin,dt
...
c
 ...
 4740 iballa(i) = 10000.*balla(i)
      abal = tmin - na*dt + x(ke)/vred
      ikom=3
      write(ioutsh,5001)x(ke),abal,ikom,npts,balmax
 5001 format(2f15.5,i5/,i10,5x,e15.4)
      write(6,4005) x(ke),abal,kom,npts,balmax
 4005 format(2f10.4,i5,i10,5x,e15.4)
      write(ioutsh,5002)(iballa(i),i=1,npts)
      write(ioutsh,5003)x(ke)
 5002 format(16i5)
 5003 format(f15.5)
 4550 continue
      return
